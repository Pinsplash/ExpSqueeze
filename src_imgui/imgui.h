...
namespace ImGui
{
    ...
    IMGUI_API void          TextWarn(const char* fmt, ...)                                  IM_FMTARGS(1); // shortcut for PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TextWarn]); Text(fmt, ...); PopStyleColor();
    IMGUI_API void          TextWarnV(const char* fmt, va_list args)                        IM_FMTLIST(1);
    ...
    IMGUI_API bool          ListBox(const char* label, int* current_item, std::vector<std::string> user_data, int items_count, int height_in_items);
